...
def _calculate_change_percent(
    ...
    if not effective_output_col_name:
        if period == 5 and price_col == "close":
            effective_output_col_name = "change_1w_percent"
        elif (period == 20 or period == 21 or period == 22) and price_col == "close":
            effective_output_col_name = "change_1m_percent"
        elif period == 1 and price_col == "close":
            effective_output_col_name = "change_1h_percent"  # Aslında günlük
        elif period == 1 and price_col == "volume":
            effective_output_col_name = "volume_change_percent_1d"
        else:
            effective_output_col_name = f"{price_col}_change_{period}bar_percent"

    try:
        if price_col not in group_df.columns:
            logger.debug(
                f"{hisse_str}: {effective_output_col_name} için '{price_col}' sütunu bulunamadı."
            )
            return pd.Series(
                np.nan, index=group_df.index, name=effective_output_col_name
            )

        if len(group_df) < period + 1:
            logger.debug(
                f"{hisse_str}: {effective_output_col_name} hesaplamak için yeterli veri yok ({len(group_df)} satır, {period} periyot gerekli)."
            )
            return pd.Series(
                np.nan, index=group_df.index, name=effective_output_col_name
            )
...
def _calculate_sma(group_df: pd.DataFrame, period: int, data_col: str) -> pd.Series:
    ...
    try:
        if data_col not in group_df.columns:
            logger.debug(
                f"{hisse_str}: {sutun_adi} için '{data_col}' sütunu bulunamadı."
            )
            return pd.Series(np.nan, index=group_df.index, name=sutun_adi)
        if len(group_df) < period:
            logger.debug(
                f"{hisse_str}: {sutun_adi} hesaplamak için yeterli veri yok ({len(group_df)} satır, {period} periyot gerekli)."
            )
            return pd.Series(np.nan, index=group_df.index, name=sutun_adi)
        return (
            group_df[data_col]
...
def _get_previous_bar_value(
    group_df: pd.DataFrame, data_col: str, shift_by: int = 1
) -> pd.Series:
    hisse_str = (
        group_df["hisse_kodu"].iloc[0]
        if not group_df.empty and "hisse_kodu" in group_df.columns
...
def _calculate_percentage_from_period_high_low(
    group_df: pd.DataFrame, price_col: str, period: int, mode: str
) -> pd.Series:
    hisse_str = (
        group_df["hisse_kodu"].iloc[0]
        if not group_df.empty and "hisse_kodu" in group_df.columns
    ...
    if period == 252 and mode == "high":
        sutun_adi = "percentage_from_52w_high"
    elif period == 252 and mode == "low":
        sutun_adi = "percentage_from_52w_low"
    else:
    ...
    try:
        ...
        if mode == "high":
            period_extreme = (
                group_df[price_col].rolling(window=period, min_periods=period).max()
            )
            percentage = (
                (group_df[price_col] - period_extreme)
                / period_extreme.replace(0, np.nan)
            ) * 100
        elif mode == "low":
            period_extreme = (
                group_df[price_col].rolling(window=period, min_periods=period).min()
            )
            percentage = (
                (group_df[price_col] - period_extreme)
                / period_extreme.replace(0, np.nan)
...
def _calculate_percentage_from_all_time_high(
    group_df: pd.DataFrame, price_col: str = "high"
) -> pd.Series:
    hisse_str = (
        group_df["hisse_kodu"].iloc[0]
        if not group_df.empty and "hisse_kodu" in group_df.columns
    ...
    sutun_adi = next(
        (
            item["name"]
            for item in config.OZEL_SUTUN_PARAMS
            if item["function"] == "_calculate_percentage_from_all_time_high"
        ),
        "percentage_from_ath_default",
    )
    try:
        if price_col not in group_df.columns:
            logger.debug(
                f"{hisse_str}: ATH'den yüzde ({sutun_adi}) için '{price_col}' sütunu eksik."
            )
            return pd.Series(np.nan, index=group_df.index, name=sutun_adi)
        all_time_high = group_df[price_col].expanding(min_periods=1).max()
        percentage_from_ath = (
            (group_df[price_col] - all_time_high) / all_time_high.replace(0, np.nan)
        ) * 100
        return percentage_from_ath.round(2).rename(sutun_adi)
    except Exception as e:
        logger.error(
            f"{hisse_str}: ATH'den yüzde ({sutun_adi}) hesaplanırken hata: {e}",
            exc_info=False,
        )
        return pd.Series(np.nan, index=group_df.index, name=sutun_adi)
