...
try:
    ...
    logger.warning("logger_setup.py bulunamadı, preprocessor.py standart logging kullanıyor.")
...
try:
    ...
    holidays = None
    logger.warning("Python 'holidays' kütüphanesi bulunamadı. Tatil günü filtrelemesi yapılamayacak.")
...
def _temizle_sayisal_deger(deger):
    ...
    if pd.isna(deger):
        return np.nan
    if isinstance(deger, (int, float, np.number)): # np.number, numpy'nin sayısal tiplerini de kapsar
        return float(deger)
    if isinstance(deger, str):
        ...
        try:
            ...
            try:
                return float(temizlenmis_deger) # Orijinal string'den sadece sayısal olmayanları attık
            except ValueError:
...
def on_isle_hisse_verileri(df_ham: pd.DataFrame, logger_param=None) -> pd.DataFrame | None:
    """
    ...
    if df_ham is None or df_ham.empty:
        fn_logger.error("Ön işleme için ham veri (df_ham) boş veya None. İşlem durduruluyor.")
        return None
    ...
    if 'tarih' not in df.columns:
        fn_logger.critical("'tarih' sütunu DataFrame'de bulunamadı. Ön işleme devam edemez.")
        return None # Kritik eksiklik, devam etmek anlamsız.

    if not pd.api.types.is_datetime64_any_dtype(df['tarih']):
        fn_logger.warning("'tarih' sütunu henüz datetime formatında değil, dönüştürülüyor (preprocessor)...")
        try:
            # Olası farklı formatları dene (genellikle pd.to_datetime bunu otomatik yapar ama garanti olsun)
            df['tarih'] = pd.to_datetime(df['tarih'], errors='coerce', dayfirst=True, infer_datetime_format=True)
            ...
            fn_logger.info("'tarih' sütunu preprocessor'da başarıyla datetime formatına dönüştürüldü.")
        except Exception as e_date_conv:
            fn_logger.error(f"'tarih' sütunu preprocessor'da da datetime'a dönüştürülemedi: {e_date_conv}. Hatalı değerler NaT olacak.", exc_info=True)
    ...
    if nat_count_initial > 0:
        fn_logger.warning(f"Dönüşüm sonrası 'tarih' sütununda {nat_count_initial} adet NaT (Not a Time) değer bulundu.")
    df.dropna(subset=['tarih'], inplace=True) # Tarih bilgisi olmayan (NaT) satırları çıkar
    rows_dropped_for_nat = nat_count_initial - df['tarih'].isnull().sum()
    if rows_dropped_for_nat > 0:
        fn_logger.info(f"Tarih sütunundaki NaT değerler nedeniyle {rows_dropped_for_nat} satır çıkarıldı.")
    if df.empty:
        fn_logger.error("Tarih sütunundaki tüm değerler NaT olduğu için DataFrame boş kaldı. Ön işleme durduruluyor.")
        return None
    ...
    if 'adj_close' in df.columns and 'adj_close' not in sayisal_hedef_sutunlar: # Eğer varsa adj_close'u da dahil et
        sayisal_hedef_sutunlar.append('adj_close')
    if 'volume_tl' in df.columns and 'volume_tl' not in sayisal_hedef_sutunlar: # Eğer varsa volume_tl'yi de dahil et (sayısal olmalı)
    ...
    for col in sayisal_hedef_sutunlar:
        if col in df.columns:
            ...
            if df[col].dtype == 'object' or pd.api.types.is_categorical_dtype(df[col]):
                ...
                df[col] = df[col].apply(_temizle_sayisal_deger).astype(float) # _temizle_sayisal_deger NaN veya float döner
                nan_after = df[col].isnull().sum()
                if nan_after > nan_before:
                     fn_logger.warning(f"'{col}' (orijinal tip: {original_type}) sütununda sayısal dönüşüm sonrası NaN sayısı arttı ({nan_before} -> {nan_after}).")
            elif not pd.api.types.is_numeric_dtype(df[col]): # Sayısal değilse ama object de değilse (örn: boolean?)
                 fn_logger.warning(f"'{col}' sütunu beklenmedik bir tipte ({df[col].dtype}), float'a dönüştürülmeye çalışılıyor.")
                 df[col] = pd.to_numeric(df[col], errors='coerce') # Hataları NaT/NaN yapar
            # Eğer zaten sayısal bir tipse ve float değilse, float'a çevir
            elif df[col].dtype != float:
                df[col] = df[col].astype(float, errors='ignore') # errors='ignore' sorunlu dönüşümlerde orijinal değeri korur (nadiren olmalı)
        else:
            ...
            if col in ['open', 'high', 'low', 'close', 'volume']:
                 fn_logger.warning(f"Ön işleme için beklenen temel sütun '{col}' DataFrame'de bulunamadı.")
    fn_logger.info("OHLCV ve volume sütunları sayısal tipe dönüştürüldü/kontrol edildi.")

    # 3. Kritik OHLC Sütunlarında NaN Varsa Satırları Çıkar
    kritik_ohlc_sutunlar = ['open', 'high', 'low', 'close'] # 'volume' NaN'ları 0 ile doldurulabilir.
    eksik_kritik_sutunlar = [s for s in kritik_ohlc_sutunlar if s not in df.columns]
    if eksik_kritik_sutunlar:
        fn_logger.error(f"Kritik OHLC sütunlarından bazıları yükleme/standardizasyon sonrası hala eksik: {eksik_kritik_sutunlar}. NaN temizliği bu sütunlar olmadan yapılamaz.")
        # Eğer temel OHLC eksikse, devam etmek anlamsız olabilir.
        if len(eksik_kritik_sutunlar) == len(kritik_ohlc_sutunlar): # Hepsi eksikse
            fn_logger.critical("Tüm kritik OHLC sütunları eksik. Ön işleme durduruluyor.")
            return None
    else:
        nan_oncesi_satir_sayisi = len(df)
        df.dropna(subset=kritik_ohlc_sutunlar, inplace=True) # how='any' (varsayılan): Herhangi birinde NaN varsa satırı çıkar
        rows_dropped_for_ohlc_nan = nan_oncesi_satir_sayisi - len(df)
        if rows_dropped_for_ohlc_nan > 0:
            fn_logger.info(f"Kritik OHLC sütunlarındaki (en az birinde) NaN değerler nedeniyle {rows_dropped_for_ohlc_nan} satır çıkarıldı.")
    ...
    if 'volume' in df.columns and df['volume'].isnull().any():
        ...
        fn_logger.info(f"'volume' sütunundaki {nan_count_volume_before_fill} adet NaN değer 0 ile dolduruldu.")

    if df.empty:
        fn_logger.error("Kritik veri temizliği (tarih, OHLC NaN'ları) sonrası DataFrame boş kaldı. Ön işleme durduruluyor.")
        return None
    ...
    if 'hisse_kodu' not in df.columns:
        fn_logger.critical("'hisse_kodu' sütunu DataFrame'de bulunamadı. Sıralama yapılamıyor. Ön işleme durduruluyor.")
        return None
    ...
    if holidays and config.TR_HOLIDAYS_REMOVE:
        try:
            ...
            if len(unique_years) > 0 and pd.notna(unique_years).all(): # Geçerli yıllar varsa
                ...
                if rows_dropped_for_holidays > 0:
                    fn_logger.info(f"BIST resmi tatil günleri nedeniyle {rows_dropped_for_holidays} satır çıkarıldı.")
                else:
                    fn_logger.info("BIST resmi tatil günleri kontrol edildi, çıkarılacak ek satır bulunamadı.")
            else:
                fn_logger.info("Veride geçerli yıl bulunamadığı veya NaN yıllar olduğu için tatil günü çıkarma işlemi atlandı.")
        except Exception as e_holiday:
            fn_logger.warning(f"Tatil günleri çıkarılırken bir hata oluştu: {e_holiday}. Bu adım atlanıyor.", exc_info=False)

    fn_logger.info(f"Ön işleme tamamlandı. Son satır sayısı: {len(df)}")
    if not df.empty:
        fn_logger.debug(f"İşlenmiş veri örneği (ilk 3 satır):\n{df.head(3).to_string()}")
    else:
