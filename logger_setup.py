...
try:
    ...
    class ConfigMock:
        ...
        LOG_DOSYA_YOLU = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "acil_durum_finansal_analiz.log"
        )

    config = ConfigMock()
    print(
        f"UYARI: config.py yüklenemedi, acil durum loglama ayarları kullanılıyor. Log dosyası: {config.LOG_DOSYA_YOLU}",
        file=sys.stderr,
    )
...
def get_logger(name="FinansalAnalizSistemi"):
    ...
    if not logger.handlers:
        ...
        try:
            ...
            )  # 'a' moduyla dosyanın sonuna ekle
            ...
            print(
                f"KRİTİK LOGGER HATASI: Log dosyası ({getattr(config, 'LOG_DOSYA_YOLU', 'Bilinmiyor')}) oluşturulamadı/yazılamadı: {e}",
                file=sys.stderr,
            )
